...
- **Kamera-Zählung (gebaut 26.09.):** Im Live-Training bei Kniebeugen, Liegestützen und Kettlebell-Swings „📷 Mit Kamera zählen“ (`ui/KameraZaehler.jsx`, Logik `utils/wiederholungZaehler.js`: Gelenkwinkel mit Hysterese, Mindestabstand 350 ms, besser sichtbare Körperseite). MediaPipe Pose Lite (`@mediapipe/tasks-vision`, nur bei Bedarf nachgeladen; WASM von jsDelivr, Modell von storage.googleapis.com), GPU mit CPU-Rückfall. Bild bleibt auf dem Gerät. Gezählte Sätze gelten als beantwortet und werden mit `satzGezaehlt` im Training gespeichert (Grundlage für faire Wettbewerbe). **Geprüft 26.09.** im Chromium der Cloud-Umgebung (Internet über den Agent-Proxy; Chromium braucht dafür `--ignore-certificate-errors-spki-list` mit dem SPKI-Pin der Proxy-CA, wie in `scripts/dauertest/tageslauf.mjs`): Erkennung lädt in ~6 s, Kamera läuft, auf dem MediaPipe-Beispielfoto (Krieger-Pose) Knie 107°/175° und Ellbogen 168°/175° – passt zum Bild. Noch offen: Test mit echter Person vor der Kamera (Nutzerin).
- **Noch offen / nächste Schritte:** Plausibilitäts-Prüfung für Wettbewerbe; Smartwatch/Health-Daten nur mit nativer App (HealthKit/Health Connect, DSGVO Art. 9). Körper-Figur neben dem Gehirn (`ui/KoerperFigur.jsx`) ist seit 26.09. live (Freigabe der Nutzerin).
...
- **Seit 25.09. keine Startseiten-Karte mehr** (Nutzerinnen-Wunsch: „nicht als permanentes Fenster im Hauptmenü“; `TagebuchKarte.jsx` liegt ungenutzt). „Wie war dein Tag?“ kommt jetzt am Abend: beim Tagebuch-Schritt der Abendroutine (`istTagebuchSchritt`: kernKey `tagebuch` oder Name enthält „Tagebuch“) im geführten Ablauf (`RoutineAblauf.jsx`, statt Timer, „Heute überspringen“ möglich) und beim Abhaken in der Checkliste (`RoutineHeuteChecklist.jsx`, erst nach Speichern abgehakt). Hat die Abendroutine keinen solchen Schritt, fragt der Abschluss-Bildschirm der Abendroutine. Manuell weiter jederzeit über `#/tagebuch`.
- **Momente festhalten** (25.09., Migration 0109 `moment_eintraege`, `ui/MomentFesthalten.jsx`): im gelben 💡-Fenster („Grad nicht gut?“) Knopf „📝 Moment festhalten“ – Gefühl (aufgewühlt, ausgerastet, impulsiv …), Stärke 1–5, Auslöser (Diktat), wer dabei, wo, was geholfen hat; danach direkt „Jetzt eine Atemübung“. Auslöser privat, außer angehakt; Coach liest über `admin_momente()` (maskiert). Anzeige auf der Tagebuch-Seite (löschbar) und beim Coach im Tagebuch-Kasten.
...
- **KI-Anbieter (Entscheidung Nutzerin 25.09.): Gemini, kostenlose Stufe.** Umstieg auf Claude erst, wenn die ganze App auf einen zahlenden Anbieter geht. Stand: Aka-Chat (`gemini-chat`, Modell aus `VITE_AI_MODEL` = gemini-3.6-flash), Lexikon + „Was hilft mir jetzt?“ (`lexikon`), Blutwerte-Foto (`blutwerte-scan`, war bis 25.09. gar nicht deployt) und Essen/Präparat-Foto (`essen-scan`) laufen alle über `GEMINI_API_KEY`. Jede Function weicht bei 404/429/500/503 automatisch auf das nächste Modell aus (gemini-flash-latest, 3.8, 3.5, 3.6). Der Claude-Weg bleibt im Code und greift automatisch, sobald `ANTHROPIC_API_KEY` als Secret gesetzt ist.
...
- **Nachgezogen (24.09. nachts):** Das Abhaken auf der Startseite (einzeln und als Supplement-Bündel) sowie „Alle bestätigen“ in Tagesplan und Supplemente-Seite schreiben jetzt „erledigt“ ins Tagesprotokoll. Vorher hatte `aenderungsprotokoll` live nur 4 Zeilen. Test: `e2e/protokoll-abhaken.spec.js`. Der Harness schreibt Aufrufe gemockter Funktionen in `window.__mockAufrufe`. Beim ersten Komplettlauf danach schlugen einmalig 2 Tests (`toHaveValue`, vermutlich das Diktier-Onboarding) fehl; drei weitere Komplettläufe und 48 Wiederholungen der Onboarding-Tests waren grün. Weiter beobachten.
...
- **Leaked-Password-Schutz bewusst aus (24.09., Entscheidung der Nutzerin):** Der Schalter ist im Gratis-Tarif ausgegraut. Der Linter-Hinweis `auth_leaked_password_protection` bleibt deshalb stehen und muss nicht erneut gemeldet werden. Aktiv sind: Passwortregel (Klein- und Großbuchstaben plus Ziffer), altes Passwort beim Ändern nötig, keine öffentliche Registrierung.
- **Registrierung gesperrt (24.09. nachts, Nutzerin in Supabase):** „Allow new users to sign up“ ist aus; öffentliches `/auth/v1/signup` liefert 422 `signup_disabled` (geprüft). Konten anlegen geht nur noch über das Admin-Dashboard bzw. die Edge Functions `admin-create-proband` und `admin-invite-proband` (live geprüft, Testkonto danach gelöscht). Neue Testkonten deshalb nur noch darüber anlegen, nicht per Signup-API.
