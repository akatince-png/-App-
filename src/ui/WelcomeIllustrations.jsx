...
export function KopfEntlastungIllustration({ size = 132 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 132 132" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="66" cy="66" r="60" style={{ fill: accentSoft }} />
      <circle cx="58" cy="52" r="24" style={{ stroke: accentDark }} strokeWidth="3.5" />
      <path d="M32 96 Q58 76 84 94" style={{ stroke: accentDark }} strokeWidth="3.5" strokeLinecap="round" fill="none" />
      <circle cx="50" cy="48" r="2.4" style={{ fill: accentDark }} opacity="0.55" />
      <circle cx="64" cy="55" r="2.4" style={{ fill: accentDark }} opacity="0.55" />
      <circle cx="56" cy="60" r="2.4" style={{ fill: accentDark }} opacity="0.55" />
...
export function UebersichtIllustration({ size = 132 }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 132 132" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {felder.map((f, i) => (
        <g key={i}>
          <rect x={f.x} y={f.y} width="30" height="30" rx="9" style={{ fill: f.fill, stroke: f.stroke }} strokeWidth="2" />
          <line x1={f.x + 8} y1={f.y + 12} x2={f.x + 22} y2={f.y + 12} style={{ stroke: f.stroke }} strokeWidth="2" strokeLinecap="round" opacity="0.7" />
          <line x1={f.x + 8} y1={f.y + 19} x2={f.x + 18} y2={f.y + 19} style={{ stroke: f.stroke }} strokeWidth="2" strokeLinecap="round" opacity="0.45" />
        </g>
      ))}
    </svg>
  );
...
export function RuheIllustration({ size = 132 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 132 132" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="66" cy="66" r="60" style={{ fill: successSoft }} />
      <circle cx="30" cy="42" r="2.6" style={{ fill: success }} opacity="0.3" />
      <circle cx="103" cy="46" r="2.2" style={{ fill: success }} opacity="0.25" />
      <circle cx="40" cy="100" r="2" style={{ fill: success }} opacity="0.22" />
      <circle cx="66" cy="66" r="28" fill="#fff" style={{ stroke: success }} strokeWidth="3.5" />
      <path d="M55 67 L63 75 L79 57" style={{ stroke: success }} strokeWidth="4.5" strokeLinecap="round" strokeLinejoin="round" fill="none" />
    </svg>
  );
}
