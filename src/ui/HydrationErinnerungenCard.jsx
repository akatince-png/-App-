...
// abfragt. Jede Zeit hat ein optionales Startdatum — die Erinnerung läuft ab
// diesem Tag dauerhaft weiter (kein einmaliger, rückwirkender Eintrag,
// sondern "ab wann soll diese wiederkehrende Erinnerung losgehen"). Leer =
// läuft schon ab heute. Geteilt zwischen dem Hydration-Onboarding-Schritt
// (Ersteingabe) und HydrationView (laufende Pflege).
export default function HydrationErinnerungenCard() {
  ...
  const [neuesStartDatum, setNeuesStartDatum] = useState("");
  ...
  const zeitHinzufuegen = () => {
    ...
    const next = [...zeiten, { zeit: neueZeit, menge: neueMenge, startDatum: neuesStartDatum }].sort((a, b) => a.zeit.localeCompare(b.zeit));
    setZeiten(next);
      ...
      {erinnerungen.hydration && (
        <div style={{ marginTop: 12 }}>
          ...
          {zeiten.map((eintrag, i) => (
            <div key={i} style={{ marginBottom: 10, paddingBottom: 10, borderBottom: `1px solid ${cardBorder}` }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 11, color: textMuted, whiteSpace: "nowrap" }}>Ab wann?</span>
                <div style={{ flex: 1 }}>
                  <TextInput type="date" value={eintrag.startDatum || ""} onChange={(v) => zeitFeldAendern(i, "startDatum", v)} />
                </div>
              </div>
              {eintrag.startDatum && eintrag.startDatum > toLocalISODate(new Date()) && (
                <div style={{ fontSize: 10.5, color: textMuted, marginTop: 2 }}>Läuft erst ab dem {eintrag.startDatum} los.</div>
              )}
            </div>
          ))}
          ...
          <div style={{ marginBottom: 6 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 11, color: textMuted, whiteSpace: "nowrap" }}>Ab wann? (optional, sonst ab heute)</span>
              <div style={{ flex: 1 }}>
                <TextInput type="date" value={neuesStartDatum} onChange={setNeuesStartDatum} />
              </div>
            </div>
          </div>
        </div>
