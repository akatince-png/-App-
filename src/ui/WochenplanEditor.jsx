...
export default function WochenplanEditor({ trainingWochenplan, trainingTemplates = [], wochenplanSetzen, wochenplanEntfernen, titel }) {
  const { t, tLabel } = useT();
    ...
    <>
      ...
      <Card style={{ marginBottom: 14 }}>
        <div style={{ fontSize: 12, color: textMuted, marginBottom: 10 }}>
          {t("onboarding.training.wochenplan.instructions")}
        </div>
        {WOCHENTAGE.map((tag) => {
          const zuweisung = trainingWochenplan.find((w) => w.wochentag === tag);
          const passendeVorlagen = trainingTemplates.filter((tpl) => tpl.art === zuweisung?.art);
          return (
            <div key={tag} style={{ padding: "10px 0", borderBottom: `1px solid ${cardBorder}` }}>
              <div style={{ fontSize: 12.5, fontWeight: 700, marginBottom: 6 }}>{tLabel(WOCHENTAGE_VOLL[tag])}</div>
              <div style={{ display: "flex", flexWrap: "wrap" }}>
                <Pill label={tLabel("Ruhetag")} selected={!zuweisung} onClick={() => wochenplanEntfernen(tag)} />
              ...
              {zuweisung && passendeVorlagen.length > 0 && (
                <div style={{ display: "flex", flexWrap: "wrap", marginTop: 6 }}>
                  {passendeVorlagen.map((tpl) => (
                    <Pill
                      key={tpl.id}
                      label={`📋 ${tpl.name}`}
                      selected={zuweisung.templateId === tpl.id}
                      onClick={() =>
                        wochenplanSetzen(tag, {
                          art: zuweisung.art,
                          templateId: zuweisung.templateId === tpl.id ? null : tpl.id,
