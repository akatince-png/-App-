...
async function anfrageOpenAiKompatibel({ system, messages, json }) {
  ...
  if (!API_KEY) {
    ...
    if (!session) throw new Error("Nicht angemeldet — für den Assistenten über Groq wird ein aktives Login benötigt.");
...
async function anfrageGemini({ system, messages, json }) {
  ...
  if (!API_KEY) {
    ...
    if (!session) throw new Error("Nicht angemeldet — für den Assistenten über Gemini wird ein aktives Login benötigt.");
